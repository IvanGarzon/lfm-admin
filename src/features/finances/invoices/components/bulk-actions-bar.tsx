    ...
    <div className="flex items-center gap-2 bg-muted/50 p-2 rounded-lg border border-border">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start">
          <DropdownMenuItem onClick={() => onUpdateStatus(selectedRows.map(r => r.original), InvoiceStatus.PENDING)}>
            <CheckCircle className="mr-2 h-4 w-4" />
            Mark as Pending
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onUpdateStatus(selectedRows.map(r => r.original), InvoiceStatus.CANCELLED)}>
