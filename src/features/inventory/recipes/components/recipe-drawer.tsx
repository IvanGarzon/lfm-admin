...
import {
  ...
  useUpdateRecipe,
  useDeleteRecipe
} from '@/features/inventory/recipes/hooks/use-recipe-queries';
...
export function RecipeDrawer({
  ...
  const isOpen = id ? (pathname?.includes(`/inventory/recipes/${id}`) ?? false) : (open ?? false);
  ...
  const handleOpenChange = useCallback(
    (openState: boolean) => {
      if (!openState) {
        ...
        if (id) {
          const basePath = '/inventory/recipes';
