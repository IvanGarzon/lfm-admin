...
export async function getVendors(
...
): Promise<ActionResult<VendorPagination>> {
  ...
  try {
    requirePermission(session.user, 'canReadVendors');

    const filters = searchParamsCache.parse(searchParams);
    const result = await vendorRepo.searchAndPaginate(filters);
 ...
 */
export async function getVendorById(id: string): Promise<ActionResult<VendorWithDetails>> {
  const session = await auth();
  if (!session?.user) {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    requirePermission(session.user, 'canReadVendors');
    const vendor = await vendorRepo.findByIdWithDetails(id);
 ...
 */
export async function getVendorStatistics(): Promise<ActionResult<VendorStatistics>> {
  const session = await auth();
  if (!session?.user) {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    requirePermission(session.user, 'canReadVendors');
    const statistics = await vendorRepo.getStatistics();
...
export async function getActiveVendors(): Promise<
  ActionResult<Array<{ id: string; vendorCode: string; name: string }>>
> {
  const session = await auth();
  if (!session?.user) {
    return { success: false, error: 'Unauthorized' };
  ...
  try {
    requirePermission(session.user, 'canReadVendors');
    const vendors = await vendorRepo.getActiveVendors();
