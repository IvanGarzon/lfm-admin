...
const PAGINATION_KEYS = new Set(['page', 'perPage', 'sort']);

/**
 * Returns true if any non-pagination filter param differs from its default.
 * Compares raw nuqs SearchParams against the feature's filter definitions,
 * skipping page/perPage/sort so those never count as active filters.
 *
 * @param rawParams - Raw search params from the URL (nuqs SearchParams)
 * @param filterDefs - The feature's searchParams definition object (from its filters file)
